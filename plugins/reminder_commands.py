...
class ReminderSystem:
    ...
    def __init__(self, bot):
        ...
        # Initialize database connections for both networks
        import os
        self.rizon_db = BotDatabase("rizon_bot.db")
        self.libera_db = BotDatabase("libera_bot.db")
    ...
    async def _check_reminders(self):
        ...
        try:
            ...
            # Check reminders from both network databases
            for network, db in [('rizon', self.rizon_db), ('libera', self.libera_db)]:
                ...
                for reminder in due_reminders:
                    ...
                    # Mark as completed in the correct database
                    db.complete_reminder(reminder_id)
                    
                    logger.info(f"Sent {network} reminder to {user} in {channel}: {message}")
...
async def set_reminder(msg, bot=None):
    ...
    try:
        ...
        network = 'libera' if msg["message"].startswith('~') else 'rizon'
        if reminder_system:
            db = reminder_system.libera_db if network == 'libera' else reminder_system.rizon_db
        else:
