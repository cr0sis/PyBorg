...
class UNOGame:
    def __init__(self, channel, starter):
        ...
        # Draw stacking state
        self.pending_draw_penalty = 0  # Number of cards to draw
        self.pending_draw_type = None  # 'draw2' or 'draw4'
        
        # Comprehensive analytics tracking per player
        self.player_analytics = {}
        
        # Game-level tracking
        self.game_start_time = None
        self.turn_start_times = {}  # Track turn start time for each player
        self.challenges_made = {}  # Track challenges by player
        self.uno_calls = {}  # Track successful UNO calls
        
        self.create_deck()

    def init_player_analytics(self, player):
        """Initialize comprehensive analytics for a player"""
        self.player_analytics[player] = {
            'cards_played': 0,
            'wild_cards_played': 0,
            'plus4_cards_played': 0,
            'plus2_cards_played': 0,
            'skip_cards_played': 0,
            'reverse_cards_played': 0,
            'successful_uno_calls': 0,
            'successful_challenges': 0,
            'failed_challenges': 0,
            'times_challenged': 0,
            'largest_hand_size': 7,  # Start with initial 7 cards
            'red_cards_played': 0,
            'blue_cards_played': 0,
            'green_cards_played': 0,
            'yellow_cards_played': 0,
            'turns_taken': 0,
            'cards_drawn_total': 0,
            'turn_times': [],  # List of turn durations in seconds
            'was_behind_when_won': False,
            'final_position': 0
        }
        self.turn_start_times[player] = None
        self.challenges_made[player] = 0
        self.uno_calls[player] = 0

    def get_comprehensive_game_data(self, player: str, position: int = 0, cards_left: int = 0, score: int = 0) -> dict:
        """Generate comprehensive game data for database analytics"""
        analytics = self.player_analytics.get(player, {})
        
        # Calculate game duration
        game_duration_seconds = 0
        if self.game_start_time:
            game_duration_seconds = int((datetime.now() - self.game_start_time).total_seconds())
        
        # Calculate average turn time
        turn_times = analytics.get('turn_times', [])
        avg_turn_time = sum(turn_times) / len(turn_times) if turn_times else 0
        fastest_turn = min(turn_times) if turn_times else 0
        longest_turn = max(turn_times) if turn_times else 0
        
        # Determine if this was a comeback win
        comeback_win = analytics.get('was_behind_when_won', False) if position == 1 else False
        uno_win = position == 1 and cards_left == 0  # Won by playing last card (true UNO win)
        
        # Calculate color preferences
        color_totals = (analytics.get('red_cards_played', 0) + analytics.get('blue_cards_played', 0) + 
                       analytics.get('green_cards_played', 0) + analytics.get('yellow_cards_played', 0))
        favorite_color = 'unknown'
        if color_totals > 0:
            colors = {'red': analytics.get('red_cards_played', 0), 'blue': analytics.get('blue_cards_played', 0),
                     'green': analytics.get('green_cards_played', 0), 'yellow': analytics.get('yellow_cards_played', 0)}
            favorite_color = max(colors.items(), key=lambda x: x[1])[0]
        
        return {
            'won': position == 1,
            'final_position': position,
            'cards_left': cards_left,
            'points_penalty': score,
            'cards_played': analytics.get('cards_played', 0),
            'wild_cards_played': analytics.get('wild_cards_played', 0),
            'plus4_cards_played': analytics.get('plus4_cards_played', 0),
            'plus2_cards_played': analytics.get('plus2_cards_played', 0),
            'skip_cards_played': analytics.get('skip_cards_played', 0),
            'reverse_cards_played': analytics.get('reverse_cards_played', 0),
            'successful_uno_calls': analytics.get('successful_uno_calls', 0),
            'successful_challenges': analytics.get('successful_challenges', 0),
            'failed_challenges': analytics.get('failed_challenges', 0),
            'times_challenged': analytics.get('times_challenged', 0),
            'game_duration_seconds': game_duration_seconds,
            'fastest_turn_seconds': fastest_turn,
            'longest_turn_seconds': longest_turn,
            'avg_turn_time_seconds': avg_turn_time,
            'largest_hand_size': analytics.get('largest_hand_size', 7),
            'comeback_win': comeback_win,
            'uno_win': uno_win,
            'red_cards_played': analytics.get('red_cards_played', 0),
            'blue_cards_played': analytics.get('blue_cards_played', 0),
            'green_cards_played': analytics.get('green_cards_played', 0),
            'yellow_cards_played': analytics.get('yellow_cards_played', 0),
            'favorite_color': favorite_color,
            'turns_taken': analytics.get('turns_taken', 0),
            'cards_drawn_total': analytics.get('cards_drawn_total', 0)
        }
    ...
    def can_stack_draw_card(self, player):
        """Check if player has matching draw cards to continue the stack"""
        if self.pending_draw_penalty == 0:
            return False
            
        player_cards = self.players.get(player, [])
        
        if self.pending_draw_type == 'draw2':
            # Can stack with any Draw 2 card
            return any(card.endswith('_draw2') for card in player_cards)
        elif self.pending_draw_type == 'draw4':
            # Can stack with any Draw 4 card  
            return any(card.endswith('_draw4') for card in player_cards)
            
        return False

    def get_stackable_draw_cards(self, player):
        """Get list of draw cards that can continue the current stack"""
        if self.pending_draw_penalty == 0:
            return []
        ...
        if self.pending_draw_type == 'draw2':
            stackable = [card for card in player_cards if card.endswith('_draw2')]
        elif self.pending_draw_type == 'draw4':
            stackable = [card for card in player_cards if card.endswith('_draw4')]
    ...
    def deal_cards(self):
        for player in self.players:
            ...
            if player not in self.player_analytics:
                self.init_player_analytics(player)
            for _ in range(7):
                card = self.draw_card()
                if card:
    ...
    def get_final_standings(self):
        ...
        logger.debug(f"Final standings calculated: {standings}")
        return standings
...
def update_uno_leaderboard(game_results, network='rizon', uno_game=None):
    """Update leaderboard with comprehensive game results using database"""
    try:
        db = BotDatabase(f"{network}_bot.db")
        ...
        for i, result in enumerate(game_results):
            ...
            # Generate comprehensive game data if UNO game instance is available
            logger.info(f"UNO analytics check: uno_game={uno_game is not None}, has_method={hasattr(uno_game, 'get_comprehensive_game_data') if uno_game else False}")
            if uno_game and hasattr(uno_game, 'get_comprehensive_game_data'):
                logger.info(f"Using comprehensive analytics for player {player}")
                game_data = uno_game.get_comprehensive_game_data(player, position, cards_left, score)
                logger.info(f"Game data for {player}: cards_played={game_data.get('cards_played', 0)}, wild_cards={game_data.get('wild_cards_played', 0)}")
                db.update_uno_stats(player, game_data)
            else:
                # Fallback to basic stats for backwards compatibility
                logger.warning(f"Using fallback basic stats for player {player} - analytics will be zero!")
                basic_game_data = {
                    'won': position == 1,
                    'final_position': position,
                    'cards_left': cards_left,
                    ...
                    'times_challenged': 0,
                    'game_duration_seconds': 0,
                    'fastest_turn_seconds': 0,
                    'longest_turn_seconds': 0,
                    'avg_turn_time_seconds': 0,
                    'largest_hand_size': 7,
                    'comeback_win': False,
                    'uno_win': position == 1 and cards_left == 0,
                    'red_cards_played': 0,
                    'blue_cards_played': 0,
                    'green_cards_played': 0,
                    'yellow_cards_played': 0,
                ...
                db.update_uno_stats(player, basic_game_data)
...
async def start_uno_game(msg, bot=None):
    ...
    uno_game.init_player_analytics(starter)
...
async def join_uno_game(msg, bot=None):
    ...
    uno_game.init_player_analytics(player)
...
async def start_uno_round(bot=None):
    ...
    uno_game.game_start_time = datetime.now()  # Record actual game start time
...
async def resolve_draw_penalty(game, bot):
    """Resolve pending draw penalty - make current player draw all accumulated cards"""
    messages = []
    current_player = game.turn_order[game.current_player]
    penalty = game.pending_draw_penalty
    
    drawn_cards = []
    for _ in range(penalty):
        card = game.draw_card()
        if card:
            game.players[current_player].append(card)
            drawn_cards.append(card)
    
    # Show the drawn cards to the affected player via NOTICE
    if bot and drawn_cards:
        cards_display = ', '.join([game.format_card(card) for card in drawn_cards])
        await bot.send_raw(f"NOTICE {current_player} :You were forced to draw: {cards_display}\r\n")
    
    messages.append(f"{current_player} draws {penalty} cards and loses their turn!")
    
    # Clear penalty and skip their turn
    game.pending_draw_penalty = 0
...
async def play_uno_card(msg, bot=None):
    ...
    # Check if player is facing a draw penalty
    if uno_game.pending_draw_penalty > 0:
        # Player can only play cards that stack on the current penalty
        card_type = card_to_play.split('_')[1]
        
        if uno_game.pending_draw_type == 'draw2' and card_type != 'draw2':
            stackable_cards = uno_game.get_stackable_draw_cards(player)
            if stackable_cards:
                cards_display = ', '.join([uno_game.card_to_play_format(card) for card in stackable_cards])
                return f"You must stack with a Draw 2 card ({cards_display}) or draw {uno_game.pending_draw_penalty} cards!"
            else:
                # No stackable cards, force them to draw
                messages = await resolve_draw_penalty(uno_game, bot)
                return messages
                
        elif uno_game.pending_draw_type == 'draw4' and card_type != 'draw4':
            stackable_cards = uno_game.get_stackable_draw_cards(player)
            if stackable_cards:
                cards_display = ', '.join([uno_game.card_to_play_format(card) for card in stackable_cards])
                return f"You must stack with a Draw 4 card ({cards_display}) or draw {uno_game.pending_draw_penalty} cards!"
            else:
                # No stackable cards, force them to draw
                messages = await resolve_draw_penalty(uno_game, bot)
                return messages

    # Check if card can be played (normal rules)
    ...
    # Track analytics for the card played
    if player in uno_game.player_analytics:
        analytics = uno_game.player_analytics[player]
        analytics['cards_played'] += 1
        
        # Track specific card types
        if card_to_play.startswith('wild_draw4'):
            analytics['plus4_cards_played'] += 1
            analytics['wild_cards_played'] += 1
        elif card_to_play.startswith('wild_'):
            analytics['wild_cards_played'] += 1
        elif card_to_play.endswith('_draw2'):
            analytics['plus2_cards_played'] += 1
        elif card_to_play.endswith('_skip'):
            analytics['skip_cards_played'] += 1
        elif card_to_play.endswith('_reverse'):
            analytics['reverse_cards_played'] += 1
            
        # Track color statistics
        if card_to_play.startswith('red_'):
            analytics['red_cards_played'] += 1
        elif card_to_play.startswith('blue_'):
            analytics['blue_cards_played'] += 1
        elif card_to_play.startswith('green_'):
            analytics['green_cards_played'] += 1
        elif card_to_play.startswith('yellow_'):
            analytics['yellow_cards_played'] += 1
    ...
    if card_type == 'skip':
        ...
        if len(uno_game.turn_order) == 2:
            # In 2-player games, reverse acts like skip - gives current player another turn
            messages.append("Reverse in 2-player: You get another turn!")
            # Don't advance turn - current player goes again
        else:
            # 3+ players: reverse direction and move to next player in new direction
            uno_game.direction *= -1
            messages.append("Direction reversed!")
            uno_game.next_player()  # Move to next player in the reversed direction
    elif card_type == 'draw2':
        # Set up or increase draw penalty
        if uno_game.pending_draw_penalty == 0:
            # Start new draw stack
            uno_game.pending_draw_penalty = 2
            uno_game.pending_draw_type = 'draw2'
        else:
            # Increase existing stack (only if same type)
            if uno_game.pending_draw_type == 'draw2':
                uno_game.pending_draw_penalty += 2
            else:
                # Can't stack draw2 on draw4, force previous penalty
                messages.extend(await resolve_draw_penalty(uno_game, bot))
                uno_game.pending_draw_penalty = 2
                uno_game.pending_draw_type = 'draw2'
        ...
        if uno_game.can_stack_draw_card(next_player):
            stackable_cards = uno_game.get_stackable_draw_cards(next_player)
            cards_display = ', '.join([uno_game.card_to_play_format(card) for card in stackable_cards])
            messages.append(f"{next_player} must draw {uno_game.pending_draw_penalty} cards OR stack with: {cards_display}")
        else:
            # Can't stack, must draw all cards
            messages.extend(await resolve_draw_penalty(uno_game, bot))
    elif card_type == 'draw4':
        # Set up or increase draw penalty
        if uno_game.pending_draw_penalty == 0:
            # Start new draw stack
            uno_game.pending_draw_penalty = 4
            uno_game.pending_draw_type = 'draw4'
        else:
            # Increase existing stack (only if same type)
            if uno_game.pending_draw_type == 'draw4':
                uno_game.pending_draw_penalty += 4
            else:
                # Can't stack draw4 on draw2, force previous penalty
                messages.extend(await resolve_draw_penalty(uno_game, bot))
                uno_game.pending_draw_penalty = 4
                uno_game.pending_draw_type = 'draw4'
    ...
    if len(uno_game.players[player]) == 0:
        # Check if game is still active to prevent double-counting
        if not uno_game.game_active:
        ...
        # Mark game as inactive BEFORE updating stats to prevent double-counting
        ...
        try:
            config = get_network_config(msg)
            network = 'libera' if msg["message"].startswith('~') else 'rizon'
            update_uno_leaderboard(standings, network, uno_game)
            
            # Increment total games counter
            db = BotDatabase(f"{network}_bot.db")
            current_total = int(db.get_stat('uno_total_games', 0))
            db.set_stat('uno_total_games', current_total + 1)
            
            # Get the correct prefix based on network
            prefix = '~' if network == 'libera' else '!'
...
async def draw_uno_card(msg, bot=None):
    ...
    # Check if player is facing a draw penalty
    if uno_game.pending_draw_penalty > 0:
        # Resolve the entire draw penalty
        messages = await resolve_draw_penalty(uno_game, bot)
        messages.append(uno_game.get_game_status_message())
        return messages
...
def show_uno_leaderboard(msg):
    ...
    try:
        ...
        # Get total games played from bot stats
        total_games = int(db.get_stat('uno_total_games', 0))
