...
class PluginManager:
    ...
    def load_plugins_from_directory(self, plugin_dir: str):
        ...
        for plugin_file in plugin_path.glob("*.py"):
            ...
            try:
                ...
                if hasattr(module, 'setup_plugin'):
                    ...
                    # Reduced verbosity - only log plugin count later
                else:
        ...
        # Summary log instead of individual plugin logs
        logger.info(f"📦 Loaded {len(self.commands)} commands from {len([f for f in plugin_path.glob('*.py') if not f.name.startswith('_')])} plugins")
...
# Global plugin manager instances (network-agnostic)
_plugin_managers = {}
...
def get_plugin_manager(network: str, command_prefix: str) -> PluginManager:
    ...
    if network not in _plugin_managers:
        _plugin_managers[network] = PluginManager(command_prefix)
        _plugin_managers[network].network = network  # Store network context
        _plugin_managers[network].load_plugins_from_directory(str(PLUGIN_DIR))
    
    return _plugin_managers[network]